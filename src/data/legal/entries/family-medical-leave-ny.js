export const FAMILY_MEDICAL_LEAVE_NY = {
  ...
  whatItMeans: {
    en: "The federal Family and Medical Leave Act provides up to 12 weeks of unpaid, job-protected leave per year for eligible workers. The employer must hold the job (or an equivalent one) and continue health insurance during the leave. New York Paid Family Leave provides up to 12 weeks of paid leave at 67% of the worker's average weekly wage, capped at 67% of the state average weekly wage (check paidfamilyleave.ny.gov for the current cap). PFL is funded by a small payroll deduction from the employee's paycheck. The two programs overlap in some situations — for example, bonding leave after a birth may qualify under both FMLA and PFL, and the employer can require them to run concurrently. For the worker's own illness or injury, FMLA provides unpaid leave while NY short-term disability insurance (DBL) provides partial wage replacement. Employers cannot retaliate against a worker for requesting or using leave under either program. The PFL premium is paid entirely by the employee through payroll deduction at a rate set annually by the state — typically a small fraction of one percent of wages, capped at a maximum annual contribution. Employers do not contribute to the PFL fund. Self-employed workers can opt into PFL by purchasing a policy directly. The two leave programs work together in specific ways: when the employer has 50 or more employees and the worker is FMLA-eligible, FMLA and PFL can run concurrently for bonding leave, exhausting both 12-week clocks at once. For larger families this matters because each parent has separate FMLA and PFL entitlements. Employer-provided benefits beyond what the law requires — additional weeks, full salary continuation, paid parental leave — are common at larger employers and are layered on top of the statutory minimums. New hires often face a waiting period: NY PFL requires 26 consecutive weeks of work before eligibility, and FMLA requires 12 months. Holiday and vacation pay during leave depends on employer policy."
  },
