export const MENTAL_HEALTH_CRISIS_RIGHTS_NY = {
  id: "mental-health-crisis-rights-ny",
  category: "health",
  tier: "state",
  jurisdiction: "us-ny",
  status: "active",

  title: { en: "Mental Health Crisis — Rights During an Emergency Hold or Evaluation" },

  summary: {
    en: "If someone is taken to a hospital during a mental health crisis, they have rights. New York law sets limits on how long a person can be held, requires a doctor's evaluation, and gives the person the right to a hearing. Knowing these rights can help during a frightening situation."
  },

  whoQualifies: {
    en: [
      "Anyone brought to a hospital or psychiatric facility during a mental health crisis, whether by police, ambulance, or on their own.",
      "Family members of someone who was taken to a hospital for a psychiatric evaluation.",
      "Anyone being held in a psychiatric emergency room and not sure of their rights.",
      "Anyone who wants to create a psychiatric advance directive before a crisis happens."
    ]
  },

  whatItMeans: {
    en: "In New York, a person in a mental health crisis can be brought to a hospital for evaluation under Mental Hygiene Law Section 9.39 (emergency admission) or Section 9.41 (police transport for evaluation). Under Section 9.39, a hospital can hold a person for up to 48 hours for evaluation if a doctor believes the person is a danger to themselves or others. If the hospital wants to keep the person longer than 48 hours, it must either get the person's consent for voluntary admission or file for involuntary commitment under Section 9.27 or 9.31. Involuntary commitment requires a court hearing within five days if the person objects. The person has the right to a lawyer at the hearing — if they cannot afford one, the Mental Hygiene Legal Service provides a free attorney. A person admitted voluntarily has the right to leave by submitting a written request. The hospital has 72 hours to either release the person or convert to involuntary status and schedule a hearing. The 988 Suicide and Crisis Lifeline (call or text 988) connects people in crisis to trained counselors and can help avoid unnecessary hospitalization."
  },

  yourRights: {
    en: [
      "A person brought to a hospital for evaluation has the right to be seen by a qualified psychiatrist.",
      "A person cannot be held longer than 48 hours under an emergency admission without consent or a court proceeding.",
      "A person facing involuntary commitment has the right to a hearing within five days and the right to a free attorney from the Mental Hygiene Legal Service.",
      "A person admitted voluntarily has the right to request release in writing. The hospital has 72 hours to respond.",
      "A person in a psychiatric facility has the right to make phone calls, send mail, and receive visitors unless there is a specific clinical reason to restrict these.",
      "A person has the right to refuse medication except in an emergency where they are an immediate danger.",
      "A person has the right to create a psychiatric advance directive that states their treatment preferences before a crisis happens."
    ]
  },

  legalOptions: {
    en: [
      "Under MHL Section 9.39, emergency admission is limited to 48 hours. Extensions require either consent or a court proceeding.",
      "The Mental Hygiene Legal Service provides free legal representation to anyone facing involuntary commitment in the Rochester area at (585) 530-3050.",
      "The 988 Suicide and Crisis Lifeline is available 24/7 by calling or texting 988. Counselors can help de-escalate a crisis and connect to local services.",
      "The Rochester FACTT (Forensic Assertive Community Treatment Team) provides community-based mental health crisis support. They can be reached through Strong Memorial Hospital.",
      "A psychiatric advance directive can be created with the help of a mental health provider or attorney. The NAMI Rochester chapter can provide information at (585) 423-1593.",
      "If a person believes their rights were violated during a hospital stay, a complaint can be filed with the NYS Office of Mental Health at (800) 597-8481."
    ]
  },

  example: {
    en: "When Aaliyah called 911 because her brother was threatening to hurt himself, police took him to Strong Memorial Hospital's psychiatric emergency room. He was held under a 9.39 emergency admission. A psychiatrist evaluated him within hours. After 36 hours, the treatment team recommended voluntary admission for further care. Her brother agreed. Aaliyah learned that if he changed his mind, he could submit a written request to leave and the hospital would have 72 hours to either release him or go to court."
  },

  counsel: [
    {
      type: "free",
      name: "Mental Hygiene Legal Service — 7th Judicial District",
      focus: "Free legal representation for people facing involuntary psychiatric commitment",
      qualifier: "Anyone facing involuntary commitment in Monroe County and surrounding area",
      access: "Phone at (585) 530-3050",
      outcome: "Legal representation at commitment hearings, patient rights advocacy",
      phone: "(585) 530-3050",
      url: "https://www.nycourts.gov/courts/ad4/MHLS.shtml",
      verified: true,
      bilingual: false,
      languages: []
    },
    {
      type: "free",
      name: "988 Suicide and Crisis Lifeline",
      focus: "24/7 crisis counseling, suicide prevention, mental health emergencies",
      qualifier: "Anyone in the U.S.",
      access: "Call or text 988, available 24/7",
      outcome: "Crisis counseling, de-escalation, connection to local services",
      phone: "988",
      url: "https://988lifeline.org",
      verified: true,
      bilingual: true,
      languages: ["es"]
    }
  ],

  relatedIds: [
    "involuntary-commitment-ny",
    "mental-health-patient-rights-ny",
    "psychiatric-advance-directive-ny",
    "patient-rights-hospital-ny",
    "narcan-access-ny"
  ],

  relatedHelpResources: [],

  tags: [
    "mental health crisis",
    "psychiatric hold",
    "involuntary commitment",
    "9.39 hold",
    "psych emergency",
    "psychiatric evaluation",
    "mental health rights",
    "hospital mental health",
    "988",
    "suicide crisis",
    "psychiatric advance directive",
    "forced hospitalization"
  ],

  sources: [
    "https://www.nysenate.gov/legislation/laws/MHY/9.39",
    "https://www.nysenate.gov/legislation/laws/MHY/9.41",
    "https://988lifeline.org"
  ],

  lastVerified: "2026-04-17",
  volatility: "low",
  emergencyFlag: true,
  disclaimer: true,
  categoryDisclaimer: null
};
