export const BANK_ACCOUNT_FROZEN_NY = {
  ...
  whatItMeans: {
    en: "Under CPLR Article 52, a creditor who wins a money judgment in court can serve a restraining notice on the debtor's bank. The bank must freeze the account immediately. The creditor then has up to one year to serve a property execution to actually take the money. Under federal and state law, certain funds are exempt from being frozen or taken. Social Security, SSI, veterans benefits, public assistance, child support received, workers' compensation, and unemployment benefits are all protected. Under the federal rule (31 CFR Part 212), if the bank can see that the last two months of deposits include protected benefits through direct deposit, the bank must automatically protect up to two months' worth of those deposits and keep them available to the account holder — even if the rest of the account is frozen. If the bank froze exempt funds, the account holder can file an exemption claim to get them released. The debtor has the right to challenge the freeze entirely if the underlying judgment is wrong. Service of process problems are at the root of many surprise account freezes. NY allows substituted service — leaving papers with someone at the home or workplace, or affixing them to the door and mailing a copy — and many debtors never receive the papers because the address used was outdated or the substitute recipient never passed the documents along. A motion to vacate the default judgment under CPLR 5015 is the way to fix this; the motion can be filed years after the judgment if the debtor was never properly served. Statutes of limitations also matter: under the Consumer Credit Fairness Act (CPLR 214-i, effective April 7, 2022), most consumer credit debts in NY now have a three-year limitation period (down from six years), and a lawsuit filed after the period has expired can be dismissed even if the underlying debt is real. Other written contracts still carry a six-year period under CPLR 213. Time-barred debts that were already reduced to judgment are still enforceable, however, because the judgment itself has its own twenty-year enforcement window."
  },
