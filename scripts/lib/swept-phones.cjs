...
const SWEPT_WRONG_PHONES = [
  ...
  { num: '585-753-6960', label: 'Monroe County DHS / DSS (old main number)',   fix: 'use (585) 753-6998 for DHS general / SNAP / TA / HEAP main, (585) 753-6316 for Child Care, (585) 753-6477 for HEAP-specific line; (585) 753-6960 IS still the appointment line for application mailing', cooccur: /(?:DHS|DSS|Human\s+Services|Social\s+Services|Department\s+of|SNAP|Food\s+Assistance|Temporary\s+Assistance|EBT|HEAP|Heating\s+Help|Cash\s+Help|cash\s+for\s+rent)/i },
  ...
  // 2026-05-04: parmany.gov/departments/town-clerk/ now shows 585-392-9461 directly
  // under Carrie Fracassi (Town Clerk / Receiver of Taxes). The 2026-04-30 sweep
  // had flagged 9461 as the main switchboard with 9462 as the clerk direct, but
  // the official site authoritatively lists 9461 as Fracassi's number today.
  // Phone-system change or stale prior verification — either way, 9461 is correct
  // per the org's own page. Flag retired.
  // { num: '585-392-9461', label: 'Parma Town Clerk', fix: '...', cooccur: /(?:Town\s+Clerk|Clerk's\s+Office)/i },
